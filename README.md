...
├── run_experiment_cluster_8bit.py   # 8-bit cluster runner (not used in final paper)
...
├── statistical_analysis.py          # Wilcoxon tests, CGG, PCS (single-seed)
├── statistical_analysis_multiseed.py # Same tests on multi-seed pooled data
...
├── results/
│   ├── cell1_baseline.json          # 8-bit seed 0 — C1 Baseline
│   ├── cell2_linguistic.json        # 8-bit seed 0 — C2 Linguistic Stress
│   ├── cell3_visual.json            # 8-bit seed 0 — C3 Visual Stress
│   ├── cell4_combined.json          # 8-bit seed 0 — C4 Full Novel
│   ├── cell1_baseline_seed1.json    # 8-bit seed 1 — C1 Baseline
│   ├── cell2_linguistic_seed1.json  # 8-bit seed 1 — C2 Linguistic Stress
│   ├── cell3_visual_seed1.json      # 8-bit seed 1 — C3 Visual Stress
│   └── cell4_combined_seed1.json    # 8-bit seed 1 — C4 Full Novel
└── figures/
    ├── fig1_2x2_results.pdf/png
    ├── fig2_heatmap.pdf/png
    ├── fig3_cgg_decomp.pdf/png
    ├── fig4_paraphrase_types.pdf/png
    └── fig5_steps_to_success.pdf/png
```

> **Files to add before committing** (see [Uploading Results](#uploading-results) below):
> - `results/cell*_seed1.json` — 8-bit seed 1 raw rollout data (from `~/VLMmodel/results/` on the laptop)
> - `results/bfloat16/cell*_seed{1,2,3}.json` — raw A100 rollout JSONs (from `~/openvla/results/` on the cluster)
>
> `per_task_results_multiseed.csv` already provides the pooled bfloat16 per-task summary used in Table 2.
...
Results are saved to `~/VLMmodel/results/`. C1/C3 run 10 rollouts per task; C2/C4 run 30 rollouts (3 variants × 10) and report the mean SR across variants as the task-level SR. Initial states are cycled if fewer than 10 are available. MuJoCo runs at 20 Hz, 256×256 px, max 300 steps per rollout; `normalize_gripper_action` + `invert_gripper_action` are applied at inference to reconcile training convention. Visual perturbation adds Gaussian noise ε~N(0, 0.05²I₇) (σ=0.05 rad per joint) to all 7 arm joints at initialisation via `sim.data.qpos[:7]`.
...
Results save to `~/openvla/results/` on the cluster node as `cell*_seed{N}.json`. The pooled per-task summary is already in `per_task_results_multiseed.csv`.
...
# Single seed only
python "Research paper/generate_figures_multiseed.py" --seeds 0
```

Outputs figures to `~/VLMmodel/Research paper/figures/`.
...
The `results/` folder in this repo currently has **seed 0 (8-bit)** data only. To fully reproduce the paper you need to add the seed 1 (8-bit) files and optionally the raw bfloat16 JSONs.

**From your laptop** (`~/VLMmodel/results/`):
```bash
cp ~/VLMmodel/results/cell1_baseline_seed1.json  results/
cp ~/VLMmodel/results/cell2_linguistic_seed1.json results/
cp ~/VLMmodel/results/cell3_visual_seed1.json     results/
cp ~/VLMmodel/results/cell4_combined_seed1.json   results/
```

**From the A100 cluster** (sync to laptop first, then):
```bash
mkdir -p results/bfloat16
cp ~/VLMmodel/results/cell*_seed{1,2,3}.json results/bfloat16/   # adjust path as needed
```

Then commit everything:
```bash
git add results/ per_task_results_multiseed.csv generate_figures_multiseed.py \
        statistical_analysis_multiseed.py run_experiment_cluster.py run_spatial_s*.sh README.md
git commit -m "Add multi-seed 8-bit and bfloat16 results; update README with final paper values"
git push
```

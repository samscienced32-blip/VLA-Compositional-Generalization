...
def find_cell_file(cell_key, seed):
    """Return path to the result file for a given cell and seed.
    Seed 0 falls back to the unseeded filename (e.g. cell1_baseline.json)
    since that is how the laptop runs saved it.
    """
    base = CELL_NAMES[cell_key]
    for fname in [f"{base}_seed{seed}.json", f"{base}.json"]:
...
def load_cell(cell_key):
    ...
    all_seeds = []
    for seed in SEEDS:
        fname = find_cell_file(cell_key, seed)
        if fname is None:
            print(f"  [WARNING] No file for cell={cell_key} seed={seed} in {RESULTS_DIR} — skipping")
            continue

...
All results are fully reproducible by re-running the scripts. C1/C3 run 10 rollouts per task; C2/C4 run 30 rollouts (3 variants × 10) and report the mean SR across variants as the task-level SR. MuJoCo runs at 20 Hz, 256×256 px, max 300 steps per rollout. Visual perturbation adds Gaussian noise ε~N(0, 0.05²I₇) (σ=0.05 rad per joint) to all 7 arm joints at initialisation.

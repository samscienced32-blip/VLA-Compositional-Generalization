...
Key differences from laptop version:
  ...
  - Paths updated for cluster scratch directory
  - --seed argument added for multi-seed runs
  - Resume support: skips cells whose output JSON already exists
  - No cv2.imshow (headless server — no display)

Usage (via Slurm — see run_experiment.sh):
  MUJOCO_GL=egl python run_experiment_cluster.py --cells 1,2,3,4 --seed 0
"""
...
# ── PATHS — cluster layout ──────────────────────────────────────
SCRATCH      = os.path.expanduser("~/openvla")
LIBERO_PATH  = os.path.expanduser("~/LIBERO")
OPENVLA_PATH = os.path.expanduser("~/openvla-oft")
RESULTS_DIR  = os.path.join(SCRATCH, "results")
FIGURES_DIR  = os.path.join(SCRATCH, "figures")
PARA_FILE    = os.path.join(os.path.dirname(os.path.abspath(__file__)), "paraphrases.json")
...
CONFIG = {
    ...
    "num_steps_wait"   : 10,
    "img_size"         : 256,
    "load_in_8bit"     : False,   # A100 has 80GB — no quantisation needed
}
...
# ── LOAD MODEL ─────────────────────────────────────────────────
print("=" * 60)
print("Loading OpenVLA (bfloat16, no quantisation — A100 80GB)...")
t0 = time.time()
...
# ── EPISODE RUNNER ─────────────────────────────────────────────
def run_episode(env, instruction, initial_state, perturb=False):
    env.reset()
    ...
    if perturb:
        try:
            ...
            sim.data.qpos[:7] += rng.normal(0, NOISE, 7)
            sim.forward()
...
# ── CELL RUNNERS ───────────────────────────────────────────────
def run_cell(cell_name, tasks_list, instruction_fn, perturb):
    results = []
    for task_idx, task in enumerate(tasks_list):
        ...
        for r in range(CONFIG["rollouts_per_task"]):
            ...
            t_ep = time.time()
            success, steps = run_episode(env, instruction, init_state, perturb=perturb)
...
if 1 in CELLS_TO_RUN:
    fname = f"cell1_baseline_seed{SEED}.json"
    if Path(f"{RESULTS_DIR}/{fname}").exists():
        print(f"\n[SKIP] Cell 1 already done ({fname})")
        c1_mean = float(np.mean([r["success_rate"] for r in json.load(open(f"{RESULTS_DIR}/{fname}"))]))
    else:
        print("\n" + "="*60 + f"\nCELL 1: BASELINE  (seed={SEED})\n" + "="*60)
...
if 2 in CELLS_TO_RUN:
    fname = f"cell2_linguistic_seed{SEED}.json"
    if Path(f"{RESULTS_DIR}/{fname}").exists():
        print(f"\n[SKIP] Cell 2 already done ({fname})")
        c2_mean = float(np.mean([r["success_rate"] for r in json.load(open(f"{RESULTS_DIR}/{fname}"))]))
    else:
        print("\n" + "="*60 + f"\nCELL 2: LINGUISTIC STRESS  (seed={SEED})\n" + "="*60)
        c2 = run_cell2(tasks)
        save(c2, fname)
...
if 3 in CELLS_TO_RUN:
    fname = f"cell3_visual_seed{SEED}.json"
    if Path(f"{RESULTS_DIR}/{fname}").exists():
        print(f"\n[SKIP] Cell 3 already done ({fname})")
        c3_mean = float(np.mean([r["success_rate"] for r in json.load(open(f"{RESULTS_DIR}/{fname}"))]))
    else:
        print("\n" + "="*60 + f"\nCELL 3: VISUAL STRESS  (seed={SEED})\n" + "="*60)
...
if 4 in CELLS_TO_RUN:
    fname = f"cell4_combined_seed{SEED}.json"
    if Path(f"{RESULTS_DIR}/{fname}").exists():
        print(f"\n[SKIP] Cell 4 already done ({fname})")
        c4_mean = float(np.mean([r["success_rate"] for r in json.load(open(f"{RESULTS_DIR}/{fname}"))]))
    else:
        print("\n" + "="*60 + f"\nCELL 4: FULL NOVEL  (seed={SEED})\n" + "="*60)
...
if all(c in CELLS_TO_RUN for c in [1, 2, 3, 4]):
    ...
    print(f"\n✅ ALL CELLS DONE in {elapsed/3600:.1f}h")
